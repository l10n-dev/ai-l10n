...
export class AiTranslator {
  ...
  private async performTranslation(
    ...
    const sourceCount = usageDetails.sourceStringsCharCount || 0;
    const glossaryCount = usageDetails.glossaryCharCount || 0;
    const terminologyCount = usageDetails.terminologyCharCount || 0;
    const instructionCount = usageDetails.instructionCharCount || 0;
    this.logger.logInfo(`  ✅ Saved to: ${outputPath}`);
    this.logger.logInfo(
      `  📊 Characters used: ${charsUsed.toLocaleString()} (content ${sourceCount.toLocaleString()}, glossary ${glossaryCount.toLocaleString()}, instruction ${instructionCount.toLocaleString()}, terminology ${terminologyCount.toLocaleString()})`,
    );
