    ...
    test("throws error when sourceFile is not provided", async () => {
      ...
      assert.ok(
        consoleErrorStub.calledWith(
          sinon.match(/Translation failed/),
          sinon.match.any,
          sinon.match({ message: sinon.match(/sourceFile is required/) }),
        ),
      );
    });
    ...
    test("throws error when source file does not exist", async () => {
      ...
      assert.ok(
        consoleErrorStub.calledWith(
          sinon.match(/Translation failed/),
          sinon.match.any,
          sinon.match({ message: sinon.match(/Source file not found/) }),
        ),
      );
    });
    ...
    test("throws error for invalid language code", async () => {
      ...
      assert.ok(
        consoleErrorStub.calledWith(
          sinon.match(/Translation failed/),
          sinon.match.any,
          sinon.match({ message: sinon.match(/Invalid language code/) }),
        ),
      );
    });
  });
    ...
    test("throws error when no languages detected and none provided", async () => {
      ...
      assert.ok(
        consoleErrorStub.calledWith(
          sinon.match(/Translation failed/),
          sinon.match.any,
          sinon.match({ message: sinon.match(/No target languages found/) }),
        ),
      );
    });
  ...
  suite("Translation - Error Handling", () => {
    ...
    test("handles null result from translation service", async () => {
      ...
      translationServiceStub.translate.resolves({
        success: false,
        reason: "unauthorized",
        message: "Translation service returned no result",
      });
    ...
    test("reports all failures in summary", async () => {
      ...
      assert.strictEqual(result.success, false);
      assert.strictEqual(result.results.filter((r) => !r.success).length, 3);
      assert.ok(consoleWarnStub.calledWith(sinon.match(/Failed: es, fr, de/)));
    });
  });
    ...
    test("uses lowest remaining balance from multiple translations", async () => {
      ...
      translationServiceStub.translate.callsFake(async () => {
        ...
        return {
          ...
          currentBalance: callCount === 1 ? 1000 : 500,
        };
      });
