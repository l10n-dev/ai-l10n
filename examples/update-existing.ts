...
async function main() {
  ...
  result.results.forEach((translation) => {
    if (translation.success) {
      console.log(
        `✅ Updated ${translation.language}: ${translation.outputPath}`
      );
      console.log(
        `   Characters used: ${translation.charsUsed?.toLocaleString()}`
      );
    } else {
      console.log(
        `❌ Failed to update ${translation.language}: ${translation.error}`
      );
    }
  });
