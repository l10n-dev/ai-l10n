...
async function main() {
  ...
  successResults.forEach(r => {
    console.log(`  - ${r.language}: ${r.charsUsed?.toLocaleString()} chars → ${r.outputPath}`);
  });
  ...
  console.log(`\n📊 Total Statistics:`);
  console.log(`  - Total characters used: ${result.totalCharsUsed.toLocaleString()}`);
  if (result.remainingBalance !== undefined) {
    console.log(`  - Remaining balance: ${result.remainingBalance.toLocaleString()} characters`);
  }
}
